...
@echo. & echo [INFO] Creating STANDARD multilingual (ENG/RUS/UKR) distro for Win7+ with .NET 4.7.1 (incl. Programming Taskbook)... & echo.
pushd "%~dp0"
...
@echo. & echo [INFO] Creating MINI (RUS-only) distro for Win7+ with .NET 4.7.1 (w/o Programming Taskbook)... & echo.
if /i {%PABCNET_QUIET_MODE%} EQU {true} (
...
@echo. & echo [%~nx0] -------- Step 3/3 -----------
@echo. & echo [INFO] Creating minimal CONSOLE distro (Mono-compatible)... & echo.
@echo. [INFO] Calling PascalABCNETConsoleZIP.bat script...
echo. & call PascalABCNETConsoleZIP.bat 2>&1 || goto :ERROR
